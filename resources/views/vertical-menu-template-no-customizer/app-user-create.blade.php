...
{{-- 
  Vista para crear nuevos usuarios en VentasFix
  
  Esta página permite a los administradores agregar nuevos usuarios al sistema.
  Incluye validación en tiempo real, generación automática de email basada en RUT,
  y validación específica para RUT chileno.
  
  Características principales:
  - Formulario con validación JavaScript
  - Generación automática de email desde RUT
        ...
        <div class="layout-page">
          ...
          <div class="content-wrapper">
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              ...
              {{-- Formulario principal de creación de usuario --}}
              <div class="card">
                <div class="card-header border-bottom">
                  {{-- Títulos dinámicos desde el controlador --}}
                  <h5 class="card-title mb-0">{{ $titulo ?? 'Crear Usuario' }}</h5>
                  <p class="card-text text-muted mt-1">{{ $subtitulo ?? 'Agregar nuevo usuario al sistema' }}</p>
                </div>
                <div class="card-body">
                  ...
                  <form method="POST" action="{{ route('usuarios.store') }}">
                    ...
                    <div class="row">
                      {{-- Campo Nombre: Validación en tiempo real para solo letras --}}
                      ...
                      {{-- Campo RUT: Validación específica para RUT chileno con dígito verificador --}}
                      <div class="col-md-6 mb-3">
                        ...
                        <div class="input-group">
                          ...
                          <span class="input-group-text">
                            <i class="ti ti-id text-muted" title="RUT chileno válido"></i>
                          </span>
                        </div>
                        @error('rut')
                          <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        {{-- Mensaje de error personalizado para RUT --}}
                        <div class="invalid-feedback" id="rut-error" style="display: none;">
                          RUT inválido. Verifique el formato y dígito verificador.
                        </div>
                        <small class="form-text text-info">
                          <i class="ti ti-info-circle me-1"></i>Ingrese RUT sin puntos pero con guión (ej: <strong>12345678-9</strong>)
                        </small>
                      </div>
                    ...
                    <div class="d-flex justify-content-between">
                      ...
                      <button type="submit" class="btn btn-primary" id="submit-btn">
                        <i class="ti ti-user-plus me-2"></i>Agregar Usuario
                      </button>
                    </div>
                  </form>
                  
                  {{-- Script de validación adicional del formulario --}}
                  <script>
                    // Validación adicional antes de enviar el formulario al servidor
                    // Esto complementa la validación HTML5 nativa del navegador
                    document.querySelector('form').addEventListener('submit', function(e) {
                      ...
                      // Verificamos que el nombre no esté vacío ni contenga solo espacios
                      if (!nombre) {
                        document.getElementById('nombre').classList.add('is-invalid');
                        document.getElementById('nombre-error').textContent = 'El nombre es requerido y no puede estar vacío';
                        document.getElementById('nombre-error').style.display = 'block';
                        hasError = true;
                      }
                      
                      // Verificamos que el apellido no esté vacío ni contenga solo espacios
                      if (!apellido) {
                        document.getElementById('apellido').classList.add('is-invalid');
                        document.getElementById('apellido-error').textContent = 'El apellido es requerido y no puede estar vacío';
                        document.getElementById('apellido-error').style.display = 'block';
                        hasError = true;
                      }
                      
                      // Verificamos que el email no esté vacío (aunque se genera automáticamente)
                      if (!email) {
                        alert('El email es requerido. Asegúrese de completar nombre y apellido para generar el email automáticamente.');
                        hasError = true;
                      }
                  ...
                  <script>
                    ...
                    function validarRutChileno(rut) {
                      // Limpiar el RUT (quitar puntos, espacios, convertir a mayúscula)
                      const rutLimpio = rut.replace(/[.\s]/g, '').toUpperCase();
                      
                      // Verificar formato básico (mínimo 8 caracteres, máximo 10)
                      ...
                      for (let i = numeroStr.length - 1; i >= 0; i--) {
                        suma += parseInt(numeroStr.charAt(i)) * multiplicador;
                        multiplicador++;
                        if (multiplicador > 7) {
                          multiplicador = 2;
                        }
                    ...
                    function formatearRut(input) {
                      let valor = input.value.replace(/[^\d\-kK]/g, ''); // Solo números, guión y K
                      
                      // Remover guiones existentes para reformatear
                      valor = valor.replace(/-/g, '');
                      
                      // Agregar guión antes del último dígito si hay al menos 7 dígitos
                      if (valor.length >= 7) {
                    ...
                    // Función para validar RUT y mostrar errores
                    function validarCampoRut() {
                      ...
                      if (valor === '') {
                        // RUT vacío es válido (campo opcional)
                    ...
                    // Script para autocompletar email
                    function autoCompletarEmail() {
                      const nombre = document.getElementById('nombre').value;
                      const apellido = document.getElementById('apellido').value;
                      const emailInput = document.getElementById('email');
                      
                      // Solo generar email si nombre y apellido son válidos
                      const nombreValido = validarSoloLetras(document.getElementById('nombre'), 'nombre-error');
                      const apellidoValido = validarSoloLetras(document.getElementById('apellido'), 'apellido-error');
                      ...
                      if (nombre && apellido && nombreValido && apellidoValido) {
                        ...
                        // Cambiar el estilo para mostrar que tiene contenido
                        if (emailGenerado.length > '@ventasfix.cl'.length) {
                          emailInput.style.backgroundColor = '#e7f3ff';
                          emailInput.style.color = '#0066cc';
                          emailInput.style.fontWeight = '500';
                        }
                      } else {
                        // Si no hay nombre o apellido válidos, limpiar email
                        emailInput.value = '';
                    ...
                    // Función para prevenir caracteres no válidos al escribir
                    ...
                    // Agregar eventos para validación en tiempo real - NOMBRE
                    nombreInput.addEventListener('input', autoCompletarEmail);
                    nombreInput.addEventListener('keypress', filtrarCaracteres);
                    ...
                    // Agregar eventos para validación en tiempo real - APELLIDO
                    apellidoInput.addEventListener('input', autoCompletarEmail);
                    apellidoInput.addEventListener('keypress', filtrarCaracteres);
                    apellidoInput.addEventListener('paste', function(e) {
                      setTimeout(() => {
                        const valor = this.value;
                        const valorLimpio = valor.replace(/[^a-zA-ZáéíóúÁÉÍÓÚñÑüÜ\s]/g, '');
                        if (valor !== valorLimpio) {
                    ...
                    // Agregar eventos para validación de RUT
                    ...
                    // También ejecutar al cargar si hay valores previos
                    autoCompletarEmail();
                    validarCampoRut();
                  </script>
                </div>
